...
class PaketPenjualan:
    ...
    def GetDataBarang(self) -> list:
        query = """
            SELECT IdBarang, NamaBarang, Satuan, HargaSatuan, JumlahMinimalBarang, JumlahMaksimalBarang, Stok FROM Barang;
        """
    ...
    def SetConstraint(self) -> None:
        for i in range(len(self.matrix)):
            self.model += self.x[self.matrix[i]['IdBarang']] >= self.matrix[i]['JumlahMinimalBarang'], f"Min Barang {self.matrix[i]['NamaBarang']}"
            self.model += self.x[self.matrix[i]['IdBarang']] <= self.matrix[i]['JumlahMaksimalBarang'], f"Max Barang {self.matrix[i]['NamaBarang']}"
